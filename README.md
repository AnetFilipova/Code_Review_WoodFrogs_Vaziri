...
3) The authors show partially expressive coding. The tissue-based prefixing convention (vs_, sp_, all_) is applied consistently across most scripts and aids readability. Some variable names are descriptive, such as rhabdias_log, oswaldocruzia_log, vs_go_sig_res_over, and contrast names like vs_WvF and sp_PH1vPH2. However, there are areas of weakness — important objects like the main gene count matrix (m), the raw count input table from StringTie (tab), and filter vectors (subg) use very short and uninformative names that give no indication of their contents or purpose. Documentation through inline comments exists but is inconsistent: some sections have clear comments explaining biological intent while others have none.

4) The code is largely not modular. Across all scripts reviewed, only one function is defined — DESeq.2.GO() in the GOseq script — and even that function is poorly implemented, relying on global variables rather than its own parameters. The vast majority of the analysis is written as long, flat scripts with repeated copy-pasted blocks. The PCA plotting script is the clearest example, where identical code is repeated three times for vs, sp, and all instead of being wrapped in a reusable function. This copy-paste pattern also appears in the DESeq2 filtering, the DGE results extraction, and the GOseq loops. This is a significant code quality issue as it makes the code harder to maintain, harder to read, and more prone to subtle bugs — as evidenced by the vs_WvF_shrink variable being silently overwritten in the DGE script.
